...
async function setupTemplate() {
  ...
  const projectName = await question('📦 Project name (astro-v5-template): ') || 'astro-v5-template';
  const projectDescription = await question('📝 Project description: ') || 'Modern Astro v5 website';
  ...
  // Feature Selection
  const includeBlog = (await question('📝 Include Blog functionality? (y/N): ')).toLowerCase() === 'y';
  const includeApi = (await question('🔌 Include API routes? (y/N): ')).toLowerCase() === 'y';
  const includeNewsletter = (await question('📧 Include Newsletter signup? (y/N): ')).toLowerCase() === 'y';
...
function removeContactFeatures() {
  try {
    ...
    rmSync(join(__dirname, 'src/components/ContactForm.astro'), { force: true });
...
function updateDependencies(includeBlog, includeApi, includeNewsletter, includeContact) {
  const packagePath = join(__dirname, 'package.json');
  const packageJson = JSON.parse(readFileSync(packagePath, 'utf-8'));
